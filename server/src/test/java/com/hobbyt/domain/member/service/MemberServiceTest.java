...
class MemberServiceTest {
	...
	void withdraw() {
		...
		memberService.withdraw(ACCESS_TOKEN, MEMBER_ID);
		...
		then(memberRepository).should(times(1)).findById(argThat(memberId -> memberId == MEMBER_ID));
	}
	...
	void update() {
		...
		memberService.updateMyInfo(MEMBER_ID, updateMyInfoRequest);
	...
	void update_password() {
		...
		memberService.updatePassword(MEMBER_ID, updatePassword);
	...
	void exception_update_password(String oldPassword, String newPassword, String checkPassword) {
		...
		assertThatThrownBy(() -> memberService.updatePassword(MEMBER_ID, updatePassword))
	...
	void update_profile() throws IOException {
		...
		memberService.updateProfile(MEMBER_ID, profileRequest, profileImage, headerImage);
